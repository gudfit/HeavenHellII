...
- **Rings (`k`‑nearest):** `W* = 2k` (unit weights on the ring).
  We also show that **sub‑threshold** (`W=2k-1`) can succeed from _some_ starts (e.g., checkerboard) — the theorem is worst‑case.
...
- **Heterogeneous friction:** `W*(τ) = max_v max(0, rest(v) − τ(v))`; the _argmax_ nodes are the bottlenecks that control the threshold.
